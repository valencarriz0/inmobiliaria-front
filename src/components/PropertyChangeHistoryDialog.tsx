...
export default function PropertyChangeHistoryDialog({ propertyId, open, onOpenChange, loadHistory = publisherPropertyService.history }: { propertyId: string; open: boolean; onOpenChange: (open: boolean) => void; loadHistory?: (id: string) => Promise<{ history: PropertyHistoryEntry[] }> }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { setHistory((await loadHistory(propertyId)).history); } catch { setError("No se pudo cargar el historial de cambios."); } finally { setLoading(false); } }, [propertyId, loadHistory]);
