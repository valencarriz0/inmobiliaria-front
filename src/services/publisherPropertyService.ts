...
import { mapPublisherProperty } from "../lib/publisher-property.ts";
import type { CreatePropertyInput, UpdatePropertyInput } from "../types/property-input.ts";
import type { PublisherProperty, PublisherPropertyDto, PropertyHistoryEntry } from "../types/publisher-property.ts";

type Response = { property: PublisherPropertyDto };
const options = () => ({ token: getAuthToken() });
const mapResponse = (response: Response) => mapPublisherProperty(response.property);
...
export const publisherPropertyService = {
  list: () => apiRequest<{ properties: PublisherPropertyDto[] }>("/publisher/properties", options()).then((response) => ({ properties: response.properties.map(mapPublisherProperty) })),
  detail: (id: string) => apiRequest<Response>(`/publisher/properties/${id}`, options()).then(mapResponse),
  create: (input: CreatePropertyInput) => withUploads(input, (images) => apiRequest<Response>("/publisher/properties", { ...options(), method: "POST", body: { ...payload(input), images } }).then(mapResponse)),
  update: (id: string, input: UpdatePropertyInput) => withUploads(input, (images) => apiRequest<Response>(`/publisher/properties/${id}`, { ...options(), method: "PATCH", body: { ...payload(input), images } }).then(mapResponse)),
