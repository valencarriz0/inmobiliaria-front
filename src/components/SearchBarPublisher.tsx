import { CURRENCIES, OPERATION_TYPES, PROPERTY_TYPES, PUBLICATION_STATUSES } from "../constants/property";
...
const SearchBarPublisher = () => {
  ...
  const { errors, fieldProps, errorId, validateForm } = useFormValidation(() => ({
    ubicacion: !ubicacion.trim() && !categoria && !tipo && !precioMax.trim() && !estado
  ...
  return (
    <form noValidate onSubmit={handleSearch} className="max-w-8xl mx-auto mb-16 p-6 border rounded-lg shadow-lg bg-white">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-6">
        ...
        <Select value={estado} onValueChange={setEstado}>
          <SelectTrigger aria-label="Estado" className="w-full h-10">
            <SelectValue placeholder="Estado" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="active">{PUBLICATION_STATUSES.active}s</SelectItem>
            <SelectItem value="paused">{PUBLICATION_STATUSES.paused}s</SelectItem>
