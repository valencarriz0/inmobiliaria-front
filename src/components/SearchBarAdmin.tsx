...
export type AdminPropertyFilters = { q: string; status: "all" | "active" | "paused" | "deleted"; operationType: "all" | "sale" | "rent" | "temporary_rent" };

export default function SearchBarAdmin({ filters, onChange }: { filters: AdminPropertyFilters; onChange: (filters: AdminPropertyFilters) => void }) {
  const id = useId();
  return <form role="search" aria-label="Buscar publicaciones" onSubmit={(event) => event.preventDefault()} className="mx-auto mb-8 rounded-lg border bg-white p-6 shadow-lg dark:bg-card">
    <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_180px_220px]">
      ...
      <div className="min-w-0 space-y-2"><Label htmlFor={`${id}-category`}>Categoría</Label><Select value={filters.operationType} onValueChange={(operationType) => onChange({ ...filters, operationType: operationType as AdminPropertyFilters["operationType"] })}><SelectTrigger id={`${id}-category`} className="h-10 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas</SelectItem><SelectItem value="sale">Venta</SelectItem><SelectItem value="rent">Alquiler</SelectItem><SelectItem value="temporary_rent">Alquiler temporario</SelectItem></SelectContent></Select></div>
    </div>
  </form>;
}
